...
rect.hclust(hc_res, k = 3, border = 2:5)

# Visualize clusters
fviz_cluster(list(data = topGenes, cluster = hc_res_3), geom = ("point"))
...
hc.cut <- cutree(hc_res_top10, k = 3)
fviz_cluster(list(data = topGenes, cluster = hc.cut), geom = ("point"))
...
hc.cut <- cutree(hc_res_top100, k = 3)
fviz_cluster(list(data = topGenes, cluster = hc.cut), geom = ("point"))
...
hc.cut <- cutree(hc_res_top1000, k = 3)
fviz_cluster(list(data = topGenes, cluster = hc.cut), geom = ("point"))
...
hc.cut <- cutree(hc_res_top10000, k = 3)
fviz_cluster(list(data = topGenes, cluster = hc.cut), geom = ("point"))
```
...
mat.z <- t(apply(topGenes_df, 1, scale))
colnames(mat.z) <- colnames(topGenes_df)

h <-Heatmap(mat.z, cluster_rows = TRUE, cluster_columns = TRUE, column_labels = colnames(mat.z), name = "Z-score", row_labels = rownames(topGenes_df))
png(filename = "results/test.png", res = 250, width = 1000, height = 2000)
print(h)
dev.off()
