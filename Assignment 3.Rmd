...
# Visualize clusters
fviz_cluster(pam_res_3, data = topGenes, geom = "point")
...
# Alluvial diagram for PAM Clustering Results
pam_alluvial_df <- as.data.frame(pam_res_1$clustering)
pam_alluvial_df <- cbind(pam_alluvial_df,
                     as.data.frame(pam_res_2$clustering),
                     as.data.frame(pam_res_3$clustering),
                     as.data.frame(pam_res_top10$clustering),
                     as.data.frame(pam_res_top100$clustering),
                     as.data.frame(pam_res_top1000$clustering),
                     as.data.frame(pam_res_top10k$clustering),
                     metadata$refinebio_specimen_part)
colnames(pam_alluvial_df) <- c("1_cluster", "2_clusters", "3_clusters", "10_genes", "100_genes", "1000_genes", "10k_genes", "brain_part")

# Put into alluvial format
pam_alluvial_df <- pam_alluvial_df %>%
                  group_by(`1_cluster`, `2_clusters`, `3_clusters`, `10_genes`, `100_genes`, `1000_genes`, `10k_genes`, `brain_part`) %>%
                  summarise(freq = n())

# Plot alluvial diagram
ggplot(pam_alluvial_df,
       aes(y = freq, axis1 = `1_cluster`, axis2 = `2_clusters`, axis3 = `3_clusters`, axis4 = `10_genes`, axis5 = `100_genes`, axis6 = `1000_genes`, axis7 = `10k_genes`)) +
  geom_alluvium(aes(fill = brain_part), width = 1/12) +
  geom_stratum(width = 1/8, fill = "black", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("1 cluster", "2 clusters", "3 clusters", "10 genes", "100 genes", "1000 genes", "10k genes"), expand = c(.02, .02)) +
  ylab("Frequency") +
  guides(fill = guide_legend(title = "Brain part")) +
  scale_fill_brewer(type = "qual", palette = "Set1") +
...
# # Prepare annotation data with PAM clustering results
# annot_df <- as.data.frame(cbind(metadata$refinebio_specimen_part,
#                                 pam_res_2$clustering,  # Replace with PAM clustering results
#                                 hc.cut))
# 
# colnames(annot_df) <- c("Brain_part", "PAM_cluster", "Hier_cluster")  # Change the annotation label for PAM clustering
# 
# # Colors were taken from https://davidmathlogic.com/colorblind/
# sample_annot <- HeatmapAnnotation(df = annot_df,
#                                   col = list(Brain_part = c("cortex" = "#E1BE6A", "hippocampus" = "#40B0A6"),
#                                              PAM_cluster = c("1" = "#1A85FF", "2" = "#D41159"),  # Update with PAM cluster colors
#                                              Hier_cluster = c("1" = "#005AB5", "2" = "#DC3220")),
#                                   annotation_label = c("Brain part", "PAM cluster", "Hierarchical cluster"))  # Update the annotation label
# 
# heatmap <- Heatmap(topGenes_hm,
#                    name = "Log-normalized counts",
#                    row_title = "Genes",
