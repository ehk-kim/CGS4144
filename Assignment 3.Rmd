...
```{r}
# Values for heatmap
test <- t(apply(topGenes_df, 1, scale))
colnames(test) <- colnames(topGenes_df)

# Sample Annotations
metadataRownames <- metadata %>%
  column_to_rownames("refinebio_accession_code") %>%
  select("refinebio_specimen_part") %>%
  rownames_to_column("refinebio_accession_code")
topGenes_df <- topGenes_df %>%
  rownames_to_column("sample")
topGenesWithClustersAndPart <- topGenes_df %>%
  inner_join(metadataRownames, by = c("sample" = "refinebio_accession_code")) %>% 
  column_to_rownames("sample")
topGenes_df <- topGenes_df %>%
  column_to_rownames("sample")

sampleAnnotations = HeatmapAnnotation(df = topGenesWithClustersAndPart, col = list(topGenesWithClustersAndPart$refinebio_specimen_part))

#rownames(metadataRownames)
#list(rownames(metadataRownames))
#sampleAnnot = HeatmapAnnotation(df = metadataRownames$refinebio_specimen_part, col = list(rownames(metadataRownames)))
#sampleAnnot
...
# Hierarchical Clustering
hierAnnot = HeatmapAnnotation(df = hc.cut)
#draw(hierAnnot)
